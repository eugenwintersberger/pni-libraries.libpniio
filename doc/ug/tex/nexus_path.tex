...
Two paths are considered as \emph{matching} if one can deduce from their
...
\end{verbatim}
It is obvious for path \cpp{a} and \cpp{b} that they reference the same object. 
The same is true for the paths \cpp{b} and \cpp{c}. Surprisingly, \cpp{a} and
\cpp{c} do not match. As \cpp{a} does not provide any type information for each 
of its nodes we cannot be sure that it references the same object as \cpp{c}. 
Thus, \emph{matching} is not the same as equality. If a \cpp{match} would be the
...
Consider a file which stores several instances of \cpp{NXdetector} within its 
\cpp{NXinstrument} group and that this file has also several entries (in other
...
returns true if the two paths \cpp{a} and \cpp{b} match and false otherwise. 
