...
namespace nx{
    ...
    */
    ...
    //! parameter but rather by its string representation. This allows easy 
    //! runtime checks. This function returns true if the object is of the 
    //! requested type, false otherwise
    /*1
    \code{.cpp}
    auto field = ....;
    ...
    */
    ...
    //! \return true if the object is of the requested type
    //!
    template<typename TT> bool check_type(const TT &o,const string &s)
    {
        return o.type_id() == type_id_from_str(s);
    }
    ...
    //! This function is particularly useful in IO routines where a type 
    //! mismatch at a particular location in the code should throw an 
    //! exception.  The last parameter is the exception record at the location 
    //! in the calling function. This shows where in your code the exception 
    //! was thrown rather than the location in check function itself (which 
    ...
    */
    ...
    //! This function is particularly useful in IO routines where a type 
    //! mismatch at a particular location in the code should throw an 
    //! exception.  The last parameter is the exception record at the 
    //! location in the calling function. This shows where in your code the 
    //! exception was thrown rather than the location in check function 
    //! itself (which is mostly unimportant). 
    ...
    */
    ...
    void check_type(const TT &o,const string &s,const exception_record &r)
    {
        ...
        {
            std::stringstream ss;
            ss<<"Object is not of type "<<type_id_from_str(s)<<"!";
            throw type_error(r,ss.str());
        }
    }
