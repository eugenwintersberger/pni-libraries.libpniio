...
BOOST_AUTO_TEST_CASE(test_get_parent)
{
  ...
  BOOST_CHECK_EQUAL(dataset.link().path(),hdf5::Path("/external_data"));
  hdf5::node::LinkTarget target = dataset.link().target();
  BOOST_CHECK_EQUAL(target.file_path(),boost::filesystem::path("RegressionIssue33_data.nxs"));
  BOOST_CHECK_EQUAL(target.object_path(),hdf5::Path("/data"));
  BOOST_CHECK_EQUAL(dataset.link().parent().link().path(),hdf5::Path("/"));
}
