...
void H5FileTest::test_creation(){
    ...
    CPPUNIT_ASSERT_THROW(file.open("H5FileTest.cpp",false),H5FileError);
    //should throw an exception because the file does not exist.
    CPPUNIT_ASSERT_THROW(file.open("blablabla.h5",false),H5FileError);

    //create a file
    CPPUNIT_ASSERT_NO_THROW(file.create("H5FileTest.h5",true,0));
    CPPUNIT_ASSERT_NO_THROW(file.close());

    //open the file
    CPPUNIT_ASSERT_NO_THROW(file.open("H5FileTest.h5",false));
...
void H5FileTest::test_assignment(){
    ...
    //for file
    H5File file1;
    CPPUNIT_ASSERT_NO_THROW(file1.create("H5FileTest.h5",true,0));
