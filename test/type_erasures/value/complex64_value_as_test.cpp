...
typedef complex64 value_type;
typedef pni::type_info<float64> sinfo_type;
typedef random_generator<value_type> generator_type;
typedef fixture<value_type> fixture_type;

BOOST_AUTO_TEST_SUITE(complex64_value_as_test)

	fixture_type create_fixture()
	{
		return fixture_type(float64(0.5)*sinfo_type::min(),
			float64(0.5)*sinfo_type::max());
	}
    ...
    BOOST_AUTO_TEST_CASE(test_as_complex32)
    {
        ...
        typedef pni::type_info<float32> tinfo_type;
        fixture_type f(float64(float32(0.5)*tinfo_type::min()),
			           float64(float32(0.5)*tinfo_type::max()));
        value v(f.value_1);
