...
BOOST_AUTO_TEST_CASE(test_abs_no_file_char)
{
  pni::nexus::Path p = pni::nexus::Path("/");
  BOOST_CHECK(p.size() == 1);
  BOOST_CHECK(is_absolute(p));
  BOOST_CHECK(p.front().first == "/");
  BOOST_CHECK(p.front().second == "NXroot");
...
BOOST_AUTO_TEST_CASE(test_abs_no_file_char_imp)
{
  pni::nexus::Path p = "/";
  BOOST_CHECK(p.size() == 1);
  BOOST_CHECK(is_absolute(p));
...
BOOST_AUTO_TEST_CASE(test_abs_no_file_string_imp)
{
  Path p = std::string("/");
  BOOST_CHECK(p.size() == 1);
  BOOST_CHECK(is_absolute(p));
  ...
  p = std::string("/./:NXentry/:NXinstrument/");
  BOOST_CHECK(p.size() == 3);
  BOOST_CHECK(is_absolute(p));
  BOOST_CHECK(p.front().first == "/");
  BOOST_CHECK(p.front().second == "NXroot");
