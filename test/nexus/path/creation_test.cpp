...
BOOST_AUTO_TEST_CASE(test_abs_no_file)
{
  ...
  BOOST_TEST(p.front().second == "NXroot");
...
BOOST_AUTO_TEST_CASE(test_abs_with_file)
{
  ...
  BOOST_TEST(p.size() == 1);
  BOOST_CHECK(is_absolute(p));
  BOOST_TEST(p.filename() == "test.nxs");

  p = Path::from_string("/data/run/test.nxs://");
  BOOST_TEST(p.size() == 1);
  BOOST_CHECK(is_absolute(p));
  BOOST_TEST(p.filename() == "/data/run/test.nxs");
...
BOOST_AUTO_TEST_CASE(from_string_without_file)
{
  ...
  BOOST_TEST(p.size() == 3);
  BOOST_CHECK(!is_absolute(p));
  BOOST_TEST(Path::to_string(p) == str);
  ...
  BOOST_CHECK(is_absolute(p));
  BOOST_TEST(p.size() == 4);
  BOOST_TEST(Path::to_string(p) == std::string(str,0,str.size()-1));
...
BOOST_AUTO_TEST_CASE(from_string_with_file_attr)
{
  ...
  Path p = Path::from_string(str);
  BOOST_TEST(p.filename() == "data/test/test.nx");
  BOOST_CHECK(is_absolute(p));
  BOOST_TEST(p.size() == 4);
  BOOST_TEST(p.attribute() == "test");
  BOOST_TEST(Path::to_string(p) == str);
