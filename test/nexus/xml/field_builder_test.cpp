...
BOOST_AUTO_TEST_CASE(test_scalar_fields)
{
  ...
  BOOST_CHECK_EQUAL(nexus::get_type_id(dataset),type_id_t::UINT8);

  dataset = hdf5::node::get_node(root_group,"/scalar_fields/uint16_field");
  BOOST_CHECK_EQUAL(nexus::get_type_id(dataset),type_id_t::UINT16);

  dataset = hdf5::node::get_node(root_group,"/scalar_fields/uint32_field");
  BOOST_CHECK_EQUAL(nexus::get_type_id(dataset),type_id_t::UINT32);

  dataset = hdf5::node::get_node(root_group,"/scalar_fields/uint64_field");
  BOOST_CHECK_EQUAL(nexus::get_type_id(dataset),type_id_t::UINT64);

  dataset = hdf5::node::get_node(root_group,"/scalar_fields/int8_field");
  BOOST_CHECK_EQUAL(nexus::get_type_id(dataset),type_id_t::INT8);

  dataset = hdf5::node::get_node(root_group,"/scalar_fields/int16_field");
  BOOST_CHECK_EQUAL(nexus::get_type_id(dataset),type_id_t::INT16);

  dataset = hdf5::node::get_node(root_group,"/scalar_fields/int32_field");
  BOOST_CHECK_EQUAL(nexus::get_type_id(dataset),type_id_t::INT32);

  dataset = hdf5::node::get_node(root_group,"/scalar_fields/int64_field");
  BOOST_CHECK_EQUAL(nexus::get_type_id(dataset),type_id_t::INT64);

  dataset = hdf5::node::get_node(root_group,"/scalar_fields/float32_field");
  BOOST_CHECK_EQUAL(nexus::get_type_id(dataset),type_id_t::FLOAT32);

  dataset = hdf5::node::get_node(root_group,"/scalar_fields/float64_field");
  BOOST_CHECK_EQUAL(nexus::get_type_id(dataset),type_id_t::FLOAT64);

  dataset = hdf5::node::get_node(root_group,"/scalar_fields/float64_field");
  BOOST_CHECK_EQUAL(nexus::get_type_id(dataset),type_id_t::FLOAT64);

  dataset = hdf5::node::get_node(root_group,"/scalar_fields/string_field");
  BOOST_CHECK_EQUAL(nexus::get_type_id(dataset),type_id_t::STRING);
}
...
BOOST_AUTO_TEST_CASE(test_multidim_fields)
{
  ...
  BOOST_CHECK_EQUAL(dataspace.size(),0);
  BOOST_CHECK_EQUAL(dataspace.type(),hdf5::dataspace::Type::SIMPLE);
  BOOST_CHECK_EQUAL(dataset.creation_list().layout(),hdf5::property::DatasetLayout::CHUNKED);
}
