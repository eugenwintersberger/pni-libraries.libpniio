...
struct get_links_recursive_test_fixture : base_fixture
{
    h5::nxgroup location;
    get_links_recursive_test_fixture():
        base_fixture("get_links_recursive_test.nxs")
    {
...
BOOST_FIXTURE_TEST_SUITE(get_links_recursive_test,get_links_recursive_test_fixture)
    ...
    BOOST_AUTO_TEST_CASE(test_simple)
    {
        ...
        auto links = get_links_recursive<link_vector>(location);
        BOOST_CHECK_EQUAL(links.size(),4);
        BOOST_CHECK_EQUAL(links[0].name(),"data");
        BOOST_CHECK_EQUAL(links[1].name(),"instrument");
        BOOST_CHECK_EQUAL(links[2].name(),"detector");
        BOOST_CHECK_EQUAL(links[3].name(),"data");
    }

    //-------------------------------------------------------------------------
    BOOST_AUTO_TEST_CASE(test_apend)
    {
        location = get_object(root,"/entry");
        link_vector links;
        get_links_recursive<link_vector>(location,links);
        BOOST_CHECK_EQUAL(links.size(),4);
        BOOST_CHECK_EQUAL(links[0].name(),"data");
        BOOST_CHECK_EQUAL(links[1].name(),"instrument");
        BOOST_CHECK_EQUAL(links[2].name(),"detector");
